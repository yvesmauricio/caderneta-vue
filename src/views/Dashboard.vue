    ...
    <div class="dash-body">
      ...
      <div class="dash-kpi-grid">
        ...
        <div class="dash-kpi-card">
          <div class="dash-kpi-label">🏪 Lojas</div>
          <div class="dash-kpi-val">{{ qtdLojas }}</div>
        </div>
        <div class="dash-kpi-card">
          <div class="dash-kpi-label">👩 Clientes</div>
          <div class="dash-kpi-val">{{ qtdClientes }}</div>
        </div>
      </div>
      ...
      <div class="dash-section">
        ...
        <div v-if="!vencimentos.length" class="empty" style="padding: 24px 0">
          <div class="empty-ico">🎉</div>
          <p>Nenhum fiado em aberto!</p>
        </div>
        <div v-else class="dash-venc-list">
          <div
            v-for="c in vencimentos" :key="c.clienteId"
            class="dash-venc-item" :class="{ urgente: c.atrasado }"
          >
            <div class="dash-venc-dot">{{ c.atrasado ? '⚠️' : '📅' }}</div>
            <div class="dash-venc-info">
              <div class="dash-venc-nome">{{ c.nome }}</div>
              <div class="dash-venc-meta">{{ c.loja }} · vence {{ fmtBR(c.minVenc) }}</div>
            </div>
            <div class="dash-venc-valor">R$ {{ fmt(c.saldo) }}</div>
          </div>
        </div>
      </div>
    </div>
...
.dash-greeting {
  ...
  padding-right: 70px; color: #fff;
  display: flex; flex-direction: column; gap: 3px;
  position: relative; flex-shrink: 0; overflow: visible;
}
.dash-greeting::after {
  content: '🍫'; position: absolute; right: 14px; top: 50%;
  transform: translateY(-50%); font-size: 42px; opacity: .18;
  pointer-events: none; line-height: 1;
}
.dash-hello      { font-size: 18px; font-weight: 800; line-height: 1.4; }
.dash-date-line  { font-size: 12px; opacity: .8; }
.dash-kpi-grid   { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; flex-shrink: 0; }
.dash-kpi-card {
  background: var(--surface); border: 1.5px solid var(--border);
  border-radius: var(--r); padding: 14px 14px; box-shadow: var(--shadow);
}
.dash-kpi-card.destaque { border-color: rgba(26,122,69,.3); background: var(--green-bg); }
.dash-kpi-card.alerta   { border-color: rgba(185,28,28,.2); background: var(--red-bg); }
.dash-kpi-label { font-size: 11px; color: var(--muted); font-weight: 600; }
.dash-kpi-val   { font-size: 18px; font-weight: 800; color: var(--brown); font-family: var(--font-mono); margin-top: 2px; }
.dash-kpi-val.verde   { color: var(--green); }
.dash-kpi-val.vermelho { color: var(--red); }
.dash-action-btn {
  ...
  box-shadow: 0 4px 14px rgba(61,32,8,.3); flex-shrink: 0;
  transition: opacity var(--t);
}
.dash-action-btn:active { opacity: .85; }
.dash-section   { display: flex; flex-direction: column; gap: 8px; }
.dash-venc-list { display: flex; flex-direction: column; gap: 8px; }
.dash-venc-item {
...
.dash-venc-item.urgente { border-color: #fecaca; background: #fff9f9; }
.dash-venc-dot  { font-size: 18px; flex-shrink: 0; }
.dash-venc-info { flex: 1; min-width: 0; }
.dash-venc-nome { font-size: 13px; font-weight: 700; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.dash-venc-meta { font-size: 11px; color: var(--muted); margin-top: 2px; }
.dash-venc-valor { font-size: 13px; font-weight: 800; font-family: var(--font-mono); color: var(--brown); flex-shrink: 0; }
</style>
